...
impl GitRecencyMapMetadata {
    /// A fixed 14-day half-life, as defined in `docs/plans/git-aware-recent-coverage-scheduler.md`.
    ...
    pub fn load_from_file(path: impl AsRef<std::path::Path>) -> Result<Self, Error> {
        let bytes = std::fs::read(path).map_err(|e| Error::from(e))?;
...
impl GitRecencyTestcaseScore {
    fn compute_decay(head_time: u64, tc_time: u64) -> f64 {
        ...
        let half_life = GitRecencyMapMetadata::HALF_LIFE_SECS as f64;
        libm::exp2(-(age / half_life))
    }
}
...
mod git_recency_tests {
    use crate::{
        HasMetadata,
        corpus::Corpus,
        corpus::{InMemoryCorpus, SchedulerTestcaseMetadata, Testcase},
        feedbacks::MapIndexesMetadata,
        inputs::NopInput,
